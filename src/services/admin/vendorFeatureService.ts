...
import { VendorOfTheMonth } from '../../entities/VendorOfTheMonth';
import { VendorCollaboration, CollabStatus } from '../../entities/VendorCollaboration';
import { InfluencerPick } from '../../entities/InfluencerPick';
import { Vendor } from '../../entities/Vendor';

const vendorOfMonthRepo = AppDataSource.getRepository(VendorOfTheMonth);
const vendorCollaborationRepo = AppDataSource.getRepository(VendorCollaboration);
const influencerPickRepo = AppDataSource.getRepository(InfluencerPick);
const vendorRepo = AppDataSource.getRepository(Vendor);

export const vendorFeatureService = {
  ...
  async getAllVendorsOfMonth(query: any) {
  ...
  async getVendorOfMonthById(id: string) {
  ...
  async updateVendorOfMonth(id: string, data: Partial<VendorOfTheMonth>) {
  ...
  async deleteVendorOfMonth(id: string) {
    const vendorOfMonth = await vendorOfMonthRepo.findOne({ 
      where: { id },
      relations: ['vendor']
    });
  ...
  async getAllCollaborations(query: any) {
    const page = parseInt(query.page) || 1;
    const limit = parseInt(query.limit) || 20;
    const skip = (page - 1) * limit;
    const status = query.status as CollabStatus;
  ...
  async updateCollaboration(id: string, data: Partial<VendorCollaboration>) {
  ...
  async getAllInfluencerPicks(query: any) {
    const page = parseInt(query.page) || 1;
    const limit = parseInt(query.limit) || 20;
    const skip = (page - 1) * limit;
  ...
  async getInfluencerPickById(id: string) {
  ...
  async updateInfluencerPick(id: string, data: Partial<InfluencerPick>) {
    const pick = await influencerPickRepo.findOne({ where: { id } });
    if (!pick) {
      throw new Error('Influencer pick not found');
    }
  ...
  async deleteInfluencerPick(id: string) {
    const pick = await influencerPickRepo.findOne({ where: { id } });
    if (!pick) {
      throw new Error('Influencer pick not found');
    }
