...
const createCategories = async (dataSource: DataSource) => {
...
const createTestVendor = async (dataSource: DataSource) => {
  ...
  if (vendor) {
    // vendor.password = await bcrypt.hash(VENDOR_PASSWORD, 10)
...
const createTestProducts = async (dataSource: DataSource, vendor: Vendor, categories: Category[]) => {
  ...
  for (const productData of products) {
    ...
    if (existingProduct) {
      // Update existing product with array data
...
export const seedTestVendor = async (dataSource: DataSource) => {
  try {
    ...
    .then(({ AppDataSource }) => {
      AppDataSource.initialize()
        .then(async (dataSource: DataSource) => {
          await seedTestVendor(dataSource);
