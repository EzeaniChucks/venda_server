...
const FashionPost_1 = require("../../entities/FashionPost");
const PostLike_1 = require("../../entities/PostLike");
const PostComment_1 = require("../../entities/PostComment");
const auth_1 = require("../../middleware/auth");
const router = (0, express_1.Router)();
router.post('/posts', auth_1.authenticate, (0, auth_1.authorize)('vendor'), async (req, res) => {
    try {
        const { caption, media, tags, postType, productId } = req.body;
        const userId = req.user.id;
        const postRepo = data_source_1.AppDataSource.getRepository(FashionPost_1.FashionPost);
...
router.post('/posts/:postId/like', auth_1.authenticate, async (req, res) => {
    try {
        const { postId } = req.params;
        const userId = req.user.id;
        const userType = req.user.role === 'vendor' ? 'vendor' : 'customer';
        const likeRepo = data_source_1.AppDataSource.getRepository(PostLike_1.PostLike);
        const postRepo = data_source_1.AppDataSource.getRepository(FashionPost_1.FashionPost);
...
router.delete('/posts/:postId/like', auth_1.authenticate, async (req, res) => {
    try {
        const { postId } = req.params;
        const userId = req.user.id;
        const userType = req.user.role === 'vendor' ? 'vendor' : 'customer';
...
router.post('/posts/:postId/comments', auth_1.authenticate, async (req, res) => {
    try {
        const { postId } = req.params;
        ...
        const commentRepo = data_source_1.AppDataSource.getRepository(PostComment_1.PostComment);
        const postRepo = data_source_1.AppDataSource.getRepository(FashionPost_1.FashionPost);
...
//# sourceMappingURL=fashion-feed.js.map